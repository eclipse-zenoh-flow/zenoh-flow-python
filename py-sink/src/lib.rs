...
#[derive(Debug)]
struct PySinkFactory(Library);

#[async_trait]
impl SinkFactoryTrait for PySinkFactory {
    async fn new_sink(
        &self,
        ctx: &mut Context,
        ...
        Ok(Some(Arc::new(PySink(my_state))))
    }
}

struct PySink(Arc<PythonState>);

#[async_trait]
impl Node for PySink {
    async fn iteration(&self) -> Result<()> {
        Python::with_gil(|py| {
            let sink_class = self.0.py_state.cast_as::<PyAny>(py)?;
...
zenoh_flow::export_sink_factory!(register);
...
fn register() -> Result<Arc<dyn SinkFactoryTrait>> {
    ...
    Ok(Arc::new(PySinkFactory(library)) as Arc<dyn SinkFactoryTrait>)
}
